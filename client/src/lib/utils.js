...
export const getNextFourTimeslots = () => {
  const timeslots = [];
  const today = dayjs(); //string jako na řádku 28 '2024-09-22 20:00:00' / dayjs()
...
export async function callBackend({ method = 'POST', url, payload, headers}) {
  const backendUrl = import.meta.env.VITE_BACKEND_BASE_URL ? `${import.meta.env.VITE_BACKEND_BASE_URL}/${url}` : `http://localhost:3500/${url}`
